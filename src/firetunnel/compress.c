...
static TcpConnection connection_s2c[256];
static TcpConnection connection_c2s[256];
...
void print_compress_table(int direction) {
	TcpConnection *conn = (direction == S2C)? &connection_s2c[0]: &connection_c2s[0];
	printf("Compression l2 table:\n");
	int i;
...
int classify(uint8_t *pkt, uint8_t *sid, int direction) {
	...
	TcpConnection *conn = (direction == S2C)? &connection_s2c[hash]: &connection_c2s[hash];
	if (conn->active) {
...
int decompress(uint8_t *pkt, int nbytes, uint8_t sid, int direction) {
	TcpConnection *conn = (direction == S2C)? &connection_s2c[sid]: &connection_c2s[sid];
	Session *s = &conn->s;
	NewHeader h;
