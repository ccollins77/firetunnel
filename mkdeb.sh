...
mv $INSTALL_DIR/usr/share/doc/firetunnel/RELNOTES $INSTALL_DIR/usr/share/doc/firetunnel/changelog.Debian
gzip -9 -n $INSTALL_DIR/usr/share/doc/firetunnel/changelog.Debian
rm $INSTALL_DIR/usr/share/doc/firetunnel/COPYING
install -m644 platform/debian/copyright $INSTALL_DIR/usr/share/doc/firetunnel/.
mkdir -p $DEBIAN_CTRL_DIR
sed "s/FIRETUNNELVER/$2/g"  platform/debian/control.$(dpkg-architecture -qDEB_HOST_ARCH) > $DEBIAN_CTRL_DIR/control
...
lintian debian.deb
mv debian.deb ../firetunnel_$2_1_$(dpkg-architecture -qDEB_HOST_ARCH).deb
cd ..
rm -fr $CODE_DIR
